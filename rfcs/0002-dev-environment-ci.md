...
This RFC defines the development environment setup and CI/CD strategy for Terrier. It covers local development tooling (devenv and process-compose), required services for development, Nix installation requirements, and the CI pipeline using self-hosted Forgejo runners with container publishing to Codeberg's registry.
...
- **devenv**: Declarative dev environment manager with automatic activation via `devenv allow`
...
1. Cloning the repo and running `devenv allow` for automatic activation
...
- Enable automatic activation with `devenv allow`
