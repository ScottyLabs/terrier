...
For developer experience, we'll use the Dioxus `dx` CLI with `--hotpatch` for subsecond hot reload. This patches functions at runtime and works independently of the Dioxus UI framework. When that's not suitable, we have mold linker, sccache, and workspace structure to manage compile times.
