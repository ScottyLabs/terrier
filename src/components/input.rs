...
pub fn Input(
    ...
    let pr_class = if input_type == "date" || input_type == "datetime-local" {
        "pr-4"
    } else {
        "pr-4"
    };
    ...
    let base_classes = format!(
        "pl-4 {} {} text-foreground-neutral-primary text-sm font-normal placeholder:text-foreground-neutral-tertiary rounded-[0.625rem]",
        pr_class, bg_class
    );
