...
/// Request a new comparison for a judge
#[cfg_attr(feature = "server", utoipa::path(
    post,
    path = "/api/hackathons/{slug}/judging/request-comparison",
    params(
        ("slug" = String, Path, description = "Hackathon slug")
    ),
    responses(
        (status = 200, description = "Comparison assigned", body = ComparisonAssignment),
        (status = 204, description = "No more comparisons available"),
        (status = 400, description = "Judging not active"),
        (status = 401, description = "Unauthorized"),
        (status = 500, description = "Server error")
    ),
    tag = "judging"
))]
#[post("/api/hackathons/:slug/judging/request-comparison", user: SyncedUser)]
pub async fn request_comparison(
    slug: String,
) -> Result<Option<ComparisonAssignment>, ServerFnError> {
    use crate::entities::{feature, pairwise_comparison, submission, teams};
    ...
    // Get all submissions with their project summaries
    // First get team IDs for this hackathon
    let team_ids: Vec<i32> = teams::Entity::find()
        .filter(teams::Column::HackathonId.eq(hackathon.id))
        .select_only()
    ...
    if team_ids.len() < 2 {
        return Ok(None); // Need at least 2 teams to compare
    }
    ...
    // Get existing comparisons for this judge
    let judge_comparisons: Vec<(i32, i32, i32)> = pairwise_comparison::Entity::find()
        .filter(pairwise_comparison::Column::JudgeId.eq(ctx.user.id))
        .select_only()
        .column(pairwise_comparison::Column::FeatureId)
        .column(pairwise_comparison::Column::SubmissionAId)
        .column(pairwise_comparison::Column::SubmissionBId)
        .into_tuple::<(i32, i32, i32)>()
        .all(&ctx.state.db)
        .await
        .map_err(|e| ServerFnError::new(format!("Failed to fetch existing comparisons: {}", e)))?;

    // Strategy: Randomly pick a feature and two teams that haven't been compared by this judge
    let mut rng = rand::thread_rng();
    let mut shuffled_features = features.clone();
    shuffled_features.shuffle(&mut rng);

    for feature_info in shuffled_features {
        let mut shuffled_submissions = submissions.clone();
        shuffled_submissions.shuffle(&mut rng);

        // Try to find a pair
        for (i, sub_a) in shuffled_submissions.iter().enumerate() {
            for sub_b in shuffled_submissions.iter().skip(i + 1) {
                // Check if judge has already compared this pair for this feature
                // We need to check both (A, B) and (B, A) directions
                let already_compared = judge_comparisons.iter().any(|(fid, sa, sb)| {
                    *fid == feature_info.id
                        && ((*sa == sub_a.id && *sb == sub_b.id)
                            || (*sa == sub_b.id && *sb == sub_a.id))
                });
                ...
                if !already_compared {
                    ...
                    let team_a = teams::Entity::find_by_id(sub_a.team_id)
                        .one(&ctx.state.db)
                        .await
                        .map_err(|e| ServerFnError::new(format!("Failed to fetch team A: {}", e)))?
                        .ok_or_else(|| ServerFnError::new("Team A not found"))?;

                    let team_b = teams::Entity::find_by_id(sub_b.team_id)
                        .one(&ctx.state.db)
                        .await
                        .map_err(|e| ServerFnError::new(format!("Failed to fetch team B: {}", e)))?
                        .ok_or_else(|| ServerFnError::new("Team B not found"))?;

                    return Ok(Some(ComparisonAssignment {
                        feature: FeatureInfo {
                            id: feature_info.id,
                            name: feature_info.name,
                            description: feature_info.description,
                        },
                        submission_a: SubmissionSummary {
                            submission_id: sub_a.id,
                            team_name: team_a.name,
                            summary: None,  // Summary not available in this context yet
                            visit_count: 0, // Not needed for comparison
                        },
                        submission_b: SubmissionSummary {
