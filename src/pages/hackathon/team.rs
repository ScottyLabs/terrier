...
use crate::components::{
    ...
    InviteMembersModal, JoinRequestModal, ModalBase, TabSwitcher, ViewTeamModal,
};
...
use crate::hackathons::handlers::teams::{
    ...
    get_all_teams, get_join_requests, get_my_invitations, get_my_team, get_outgoing_join_requests,
    kick_member, leave_team, leave_team_force, reject_join_request, transfer_ownership,
};
                            ...
                            rsx! {
                                if is_empty {
                                    ...
                                    div { class: "divide-y divide-stroke-neutral-1",
                                        ...
                                        if !has_team {
                                            if let Some(Some(requests)) = outgoing.as_ref() {
                                                for request in requests {
                                                    OutgoingRequestCard {
                                                        key: "out-{request.id}",
                                                        request: request.clone(),
            ...
            if let Some(Some(team)) = &*my_team.read() {
                {
                    ...
                    let member_text = if other_members == 1 {
                        format!("the 1 other member")
                    } else {
                        format!("all {} members", other_members)
                    };
                    rsx! {
                        ModalBase {
                            ...
                            div { class: "flex gap-3 justify-end",
                                ...
                                Button {
                                    ...
                                    onclick: move |_| {
                                        ...
                                        let mut outgoing_requests = outgoing_requests.clone();
                                        let mut my_invitations = my_invitations.clone();
                                        let mut my_team = my_team.clone();
                                        let mut all_teams = all_teams.clone();
                                        spawn(async move {
